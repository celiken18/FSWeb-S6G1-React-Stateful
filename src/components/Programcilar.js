...
export default function Programcilar() {
  ...
  const oneCikaninIsmi = (id) => {
    setOneCikaninIdsi(id);

    return programcilarListesi.find((dev) => dev.id === oneCikaninIdsi).isim;
  };
  // Bunu sona bırakın!
  ...
  const stil = {
    fontSize: "1.5em",
    marginTop: "0.5em",
    color: oneCikaninIdsi ? "gold" : "royalblue", // 🤔 kutlarken renk gold'a dönecek
  };
